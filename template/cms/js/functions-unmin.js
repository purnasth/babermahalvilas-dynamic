...
$(document).ready(function() {
    ...
    var ele = $('h2, h1, #primary-menu ul li a, .brand, .hd');
    var wd;
	...
	var htm = '';
	$.each(wd, function(){
	   htm	+= '<span style="font-size:150%">'+this.substring(0,1)+'</span>'+this.substring(1)+' ';
	})
	//console.log($(e));
	$(this).html(htm);
...
(function($){
	...
	IGNITE.initialize = {
        ...
        fullScreen: function(){
            if( $fullScreenEl.length > 0 ) {
                $fullScreenEl.each( function(){
                    ...
                    if( element.attr('id') == 'slider' && !element.hasClass('sempera-slider-grid') ) { if( element.has('.swiper-slide') ) { element.find('.swiper-slide').css('height', scrHeight); } }
                });
            }
        },
			...
			if( $commonHeightEl.length > 0 ) {
				$commonHeightEl.each( function(){
					...
					if( element.has('.common-height') ) {
						IGNITE.initialize.commonHeight( element.find('.common-height') );
					}

					IGNITE.initialize.commonHeight( element );
				});
			}
		},
        ...
        //             transition: function(url){ /*window.location.reload();*/ window.location.href = url; }
        //         });
        //     }
        // },
	...
	IGNITE.header = {
		...
		init: function(){
			...
			IGNITE.header.overlayMenu();
			IGNITE.header.stickyMenu();
			IGNITE.header.sideHeader();
			IGNITE.header.sidePanel();
			...
			if( $onePageMenuEl.length > 0 ){
				...
				$onePageMenuEl.find('a[data-href]').click(function(){
					...
					if( $( divScrollToAnchor ).length > 0 ) {
						...
						if( !onePageOffset ) {
							var onePageOffsetG = IGNITE.initialize.topScrollOffset();
						} else {
		...
		loadFlexSlider: function(){
			...
			if( $flexSliderEl.length > 0 ){
				$flexSliderEl.each(function() {
					...
					if( !flexsAnimation ) { flexsAnimation = 'fade'; }
					...
					if( !flexsPause ) { flexsPause = 4000; }
					if( !flexsSpeed ) { flexsSpeed = 1000; }
					if( !flexsVideo ) { flexsVideo = false; }
					if( flexsDirection == 'vertical' ) { flexsSheight = false; }
					if( flexsPagi == 'false' ) { flexsPagi = false; } else { flexsPagi = true; }
					if( flexsThumbs == 'true' ) { flexsPagi = 'thumbnails'; } else { flexsPagi = flexsPagi; }
					if( flexsArrows == 'false' ) { flexsArrows = false; } else { flexsArrows = false }
					if( flexsHover == 'false' ) { flexsHover = false; } else { flexsHover = false; }
		...
		linkScroll: function(){
			$("a[data-scrollto]").click(function(){
				var element = $(this),
					...
					if( !divScrollOffset ) { divScrollOffset = IGNITE.initialize.topScrollOffset(); }
	...
	IGNITE.documentOnLoad = {
		...
		init: function(){
			...
			IGNITE.initialize.testimonialsGrid();
			IGNITE.initialize.verticalMiddle();
			IGNITE.initialize.stickFooterOnSmall();
			IGNITE.portfolio.portfolioDescMargin();
			IGNITE.portfolio.arrange();
