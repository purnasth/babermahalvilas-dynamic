...
function getMenuList($caption="", $link="", $type=0, $class=""){
	$splitSRC   = explode("http://", $link);
	$classLink 	= ($class == '') ? '' : 'class="'.$class.'"';
	$linkhref  = (count($splitSRC) == 1)? BASE_URL.$link : $link;
	...
	return "<a class='menu_color' href=\"".$linkhref."\" ".$classLink.$idLink.$linkType."><div>".$caption."</div></a>";
}
...
$replace = '<object width="550" height="350">
    ...
    <embed src="http://www.youtube.com/v/$1?fs=1"
        type="application/x-shockwave-flash" allowscriptaccess="always" width="560" height="315">
    </embed>
    </object>';
