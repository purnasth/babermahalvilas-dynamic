<?php 
	...
	{			
		case "add":	
			...
			$checkDupliName=Testimonial::checkDupliName($record->name);			
			if($checkDupliName):
				echo json_encode(array("action"=>"warning","message"=>"Testimonial Name Already Exists."));		
				exit;		
			endif;
		...
		case "edit":
			...
			if($record->name!=$_REQUEST['name']){
				$checkDupliName=Testimonial::checkDupliName($_REQUEST['name']);
				if($checkDupliName):
					echo json_encode(array("action"=>"warning","message"=>"Testimonial Name already exist."));		
					exit;		
				endif;
			}
