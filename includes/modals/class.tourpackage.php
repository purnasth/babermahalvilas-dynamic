<?php
class Tourpackage extends DatabaseObject {

	protected static $table_name = "tbl_tourpackage";
	protected static $db_fields = array('id', 'title', 'name', 'content', 'status', 'sortorder', 'image');
	...
	//GET ALL Tourpackage 
	public static function get_alltourpackage(){
		global $db;
	...
	public static function get_tourpackage_link($Lsel='',$LType=0)
	{
		global $db;		
		...
		$result='';		
		if($pages):
		$result.='<optgroup label="Tour Packages">';
			foreach($pages as $pageRow):
				$sel = ($Lsel==("tour-package".$pageRow->id."-".$pageRow->name)) ?'selected':'';
				$result.='<option value="tour-package'.$pageRow->id.'-'.$pageRow->name.'" '.$sel.'>&nbsp;&nbsp;'.$pageRow->title.'</option>';
			endforeach;
		$result.='</optgroup>';	
		endif;
		return $result;
...
//	}
	...
	public static function getTotalTourpackage(){
		global $db;
