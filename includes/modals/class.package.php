...
class Package extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'image', 'banner_image', 'title', 'sub_title', 'status', 'sortorder', 'detail', 'facility', 'content', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'dis_rate', 'added_date');
	...
	static function getRoompackage($limit='') {
		global $db;
		$cond = !empty($limit)?' LIMIT '.$limit :'';
	...
	{
		global $db;
		$sql = "SELECT title FROM ".self::$table_name." WHERE title='$title' LIMIT 1";
		$query = $db->query($sql);
	...
	{
		global $db;		
		$sql = "SELECT id, slug, title FROM ".self::$table_name." ORDER BY sortorder ASC";
		$pages = self::find_by_sql($sql);		
		$linkpageDis = ($Lsel==1)?'hide':'';
		
		$result='';		
		if($pages): 
		$result.='<optgroup label="Packages">';
			foreach($pages as $pageRow):
				$chkChild = Subpackage::getTotalSub('"'.$pageRow->id.'"');
				...
				if($subrec){
					foreach($subrec as $rows){
						$sel2 = ($Lsel==("subpackage/".$rows->slug)) ?'selected':'';
						$result.='<option value="subpackage/'.$rows->slug.'" '.$sel2.'>&nbsp; &nbsp;---'.$rows->title.'</option>';
					}
				}
			endforeach;
		$result.='</optgroup>';	
