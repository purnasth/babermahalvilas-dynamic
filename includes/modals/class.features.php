...
class Features extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'title', 'parentId', 'image', 'status', 'sortorder', 'added_date');
	...
	static function find_all_byparnt($parentId=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE parentId=$parentId  ORDER BY sortorder DESC";
	...
	static function get_by_id($id=0){
		global $db;
		$result_array = self::find_by_sql("SELECT title,image FROM ".self::$table_name." WHERE id={$id} LIMIT 1");
